...
export class ExchangeAdapter {
  ...
  /**
   * Set stop loss and take profit for spot trading using algo orders
   * 现货交易使用策略委托设置止盈止损
   */
  async setSpotStopLossAndTakeProfit(
    symbol: string,
    quantity: number,
    stopPrice?: number,
    ...
    try {
      // 现货交易使用 BTC-USDT 格式，不是合约的 BTC/USDT:USDT 格式
      const spotSymbol = `${symbol}-USDT`;
      
      // 获取当前最新价格，用于验证止盈止损价格的合理性
      const ticker = await this.exchange.fetchTicker(spotSymbol);
      const currentPrice = ticker.last || ticker.close || 0;
      
      console.log(`[SPOT] Current ${symbol} price: ${currentPrice}`);
      console.log(`[SPOT] Stop loss: ${stopPrice}, Take profit: ${takeProfitPrice}`);
      
      // 验证价格合理性
      const errors: string[] = [];
      
      if (stopPrice && stopPrice >= currentPrice) {
        const msg = `Stop loss price ${stopPrice} must be less than current price ${currentPrice}`;
        console.error(`[SPOT] ${msg}`);
        errors.push(msg);
      }
      
      if (takeProfitPrice && takeProfitPrice <= currentPrice) {
        const msg = `Take profit price ${takeProfitPrice} must be greater than current price ${currentPrice}`;
        console.error(`[SPOT] ${msg}`);
        errors.push(msg);
      }
      
      // 如果有价格验证错误，抛出异常让 AI 重试
      if (errors.length > 0) {
        throw new Error(`Invalid stop loss/take profit prices: ${errors.join('; ')}`);
      }
      
      // 如果没有设置任何止盈止损，直接返回
      if (!stopPrice && !takeProfitPrice) {
        console.log(`[SPOT] No stop loss or take profit prices provided, skipping`);
        return { data: [] };
      }
      
      // 使用 OKX 的策略委托 API (order-algo)
      // 参考: https://www.okx.com/docs-v5/zh/#order-book-trading-algo-trading-post-place-algo-order
      
      const params: any = {
        instId: spotSymbol,  // 现货格式：BTC-USDT
        tdMode: 'cash',  // 现货模式
        side: 'sell',    // 卖出（平仓）
      ...
      // 设置止损
      if (stopPrice) {
        params.slTriggerPx = stopPrice.toString();
        params.slOrdPx = '-1';  // -1 表示市价单
      }
      
      // 设置止盈
      if (takeProfitPrice) {
        params.tpTriggerPx = takeProfitPrice.toString();
        params.tpOrdPx = '-1';  // -1 表示市价单
      }
      
      // 如果同时设置止盈和止损，使用 oco 类型（双向止盈止损）
      if (stopPrice && takeProfitPrice) {
        params.ordType = 'oco';
      }
      
      // 调用 OKX 的策略委托 API
      // 使用 ccxt 的通用 API 调用方法
      const result = await (this.exchange as any).privatePostTradeOrderAlgo(params);
      
      console.log(`[SPOT] Set stop loss/take profit for ${symbol}:`, {
        stopPrice,
        takeProfitPrice,
        quantity,
        result
      ...
      console.error(`Error setting spot stop loss/take profit for ${symbol}:`, error);
      throw error;
    }
  }
